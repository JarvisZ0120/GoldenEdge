...
class ADXGridM1:
    def __init__(self, df, timeperiod=20):
        """
        初始化 ADXIndicator 类，用于计算 ADX 指标。

        参数：
            df: pandas DataFrame, 必须包含 'high', 'low', 'close' 列
            timeperiod: int, 计算 ADX 的周期（默认 20）
        """
        required_cols = ['high', 'low', 'close']
        if not all(col in df.columns for col in required_cols):
            print(f"⚠️ DataFrame 缺失 'high', 'low', 'close' 列！")
    ...
    def is_range_market(self, threshold=25):
        """
        判断市场是否为震荡市场 (范围盘)，
        当 ADX 值低于设定阈值 (例如 25) 时，认为市场处于震荡状态。

        参数：
            threshold: float, 判断震荡市场的 ADX 阈值 (默认 25)
        
        返回：
            bool: 若 ADX < threshold, 则返回 True (震荡市场)，否则返回 False (趋势市场)
        """
        adx_value = self.get_adx()
        return adx_value < threshold
    
    def is_trending_market(self, threshold=25):
        """
        判断市场是否为趋势行情。
        当 ADX 值大于设定阈值时，认为市场处于趋势行情。

        参数：
            threshold: float, 判断趋势市场的 ADX 阈值 (默认 25)
